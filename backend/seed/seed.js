 ...
 * Seed script
 *
 * Modes (controlled by SEED_MODE env var):
 *   SEED_MODE=fake  → Insert static fixture data for local dev/demo (no API calls)
 *   (default)       → Run the full football-data.org sync pipeline via fdSyncService
 *
 * Usage:
 *   SEED_MODE=fake node seed/seed.js        # fast local dev
 *   node seed/seed.js                       # real sync (requires FOOTBALL_DATA_API_KEY)
 */
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    if (Math.random() < 0.18) {
      ...
      ratings.push({
        ...
        sofascoreRating:  dataQ !== 'event' ? Math.round(r * 10) / 10 : null,
        normalisedCustom: dataQ === 'event' ? Math.round(r * 10) / 10 : null,
        dataQuality: dataQ,
      });
    }
  }
...
async function runFakeSeed() {
  console.log('[Seed] SEED_MODE=fake — inserting static fixture data (no API calls)');
  ...
  const playerMap = {};
  players.forEach((p, i) => {
    playerMap[p.slug] = { ...p.toObject(), baseRating: PLAYERS_DATA[i].rating, dataQ: PLAYERS_DATA[i].dataQ };
  });
  ...
  function closestMatch(leagueId, ratingDate) {
    ...
    let best = arr[0], minDiff = Math.abs(new Date(arr[0].kickoffUtc) - ratingDate);
    for (const m of arr) {
  ...
  for (const p of players) {
    ...
    const history = genRatings(pd.baseRating, 90, pd.dataQ);
  ...
  console.log('[Seed] Fake seed complete!');
}

// ── Real sync ─────────────────────────────────────────────────────────────────

async function runRealSync() {
  console.log('[Seed] Real mode — running football-data.org sync pipeline');
  console.log('[Seed] Tip: export SEED_MODE=fake to use static fixture data instead');
  const { syncAll } = require('../services/fdSyncService');
  await syncAll({ includeSquads: true });
